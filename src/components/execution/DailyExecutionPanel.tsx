...
export function DailyExecutionPanel({ date, childId, templates }: { date: Date; childId: string; templates: TaskTemplate[] }) {
  ...
  useEffect(() => { queueMicrotask(() => { const tasks = loadScheduledTasks(childId, dateKey); setScheduled(tasks); setLedger(loadCompletionLedger(childId, dateKey)); setSelectedId(tasks[0]?.id ?? null); }); }, [childId, dateKey]);
