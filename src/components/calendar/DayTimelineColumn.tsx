...
export function DayTimelineColumn({ date, schedules, tasks = [] }: { date: Date; schedules: FixedSchedule[]; tasks?: WeeklyCalendarFakeTask[] }) {
  const freeBlocks = getFreeTimeBlocks(schedules);
  const href = `/day/${formatLocalDate(date)}`;
  return <Link href={href} aria-label={`Open plan for ${date.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}`} className="relative block h-[600px] overflow-hidden rounded-[18px] border border-[#f0e5d7] bg-[#fffdf9] outline-offset-[-3px] transition hover:border-[#dec9b3] focus-visible:outline-2 focus-visible:outline-amber-500">
